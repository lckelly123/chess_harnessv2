...
`Opportunities`, and `Synthesis`. Every pass must call the native `agent_step`
function with complete updated notes in `running_thoughts` and a non-empty
`tool_calls` array. Do not write tool calls or notes as assistant text.
...
Batch compatible actions inside one `agent_step` whenever the batch rules allow.
On the response after creating a branch, annotate it in the same `tool_calls`
array as the next compatible action instead of spending a separate response
only on annotation.
...
Include `submit_move` in `agent_step.tool_calls` when comparison is complete.
It accepts `move`, `tested_branch`, and `decision_summary`.
`move` must be a canonical legal SAN
